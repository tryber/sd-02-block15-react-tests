...
describe('Pokemon About.js tests', () => {
  test('21 - A página "About" deve exibir informações sobre a Pokédex', () => {
    ...
    expect(queryByText('About Pokédex', { selector: 'h2' }));
    // A página deve conter dois parágrafos com texto sobre a Pokédex
    expect(queryByText('This application simulates a Pokédex, a digital encliclopedia containing all Pokémons ', { selector: 'p' }));
    expect(queryByText('One can filter Pokémons by type, and see more details for each one of them', { selector: 'p' }));
